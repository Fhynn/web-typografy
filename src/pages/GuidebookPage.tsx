  ...
  return (
    <div ref={pageRef} className="min-h-screen bg-[#060606] text-[#F0EDE8]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav className="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-6 sm:px-12 py-6">
        ...
        <span className="text-[10px] tracking-[0.18em] font-medium" style={{ color: 'rgba(240,237,232,0.25)' }}>GUIDEBOOK</span>
      </nav>
      ...
      <header ref={headerRef} className="pt-32 sm:pt-40 pb-16 px-6 sm:px-12 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <h1 data-h className="font-light leading-[0.95] tracking-[-0.035em]" style={{ fontSize: 'clamp(42px, 6vw, 88px)' }}>
            <span className="block">Your pocket expert,</span>
            <span className="block" style={{ color: 'rgba(240,237,232,0.28)' }}>wherever you land.</span>
          </h1>
      ...
      <main className="px-6 sm:px-12 max-w-7xl mx-auto pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
          ...
          >
            ...
            <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(6,6,6,0.06) 0%, rgba(6,6,6,0.55) 100%)' }} />
            {/* Overlay info */}
            <div className="absolute bottom-0 left-0 right-0 p-7 sm:p-9">
              <p className="text-[10px] tracking-[0.2em] font-medium mb-2" style={{ color: 'rgba(240,237,232,0.45)' }}>
                {GUIDES[activeGuide].id} OF 03
              </p>
              <h2 className="font-light leading-[1.0] tracking-[-0.025em] text-[#F0EDE8]" style={{ fontSize: 'clamp(26px, 3vw, 42px)' }}>
                {GUIDES[activeGuide].title}
              </h2>
              <p className="mt-1 text-[13px]" style={{ color: 'rgba(240,237,232,0.45)' }}>
          ...
          <div ref={guideRef} className="flex flex-col" style={{ gap: 2 }}>
            {GUIDES.map((g, i) => (
              <div
                ...
                style={{
                  background: activeGuide === i ? 'rgba(240,237,232,0.06)' : 'rgba(240,237,232,0.02)',
                  border: `1px solid ${activeGuide === i ? 'rgba(240,237,232,0.12)' : 'rgba(240,237,232,0.04)'}`,
                }}
                onMouseEnter={() => setActiveGuide(i)}
              >
                <div className="flex items-start gap-5">
                  <span className="text-[11px] font-light mt-0.5" style={{ color: 'rgba(240,237,232,0.25)' }}>{g.id}</span>
                  <div>
                    <h3 className="text-[16px] sm:text-[18px] font-light tracking-[-0.01em] mb-1">{g.title}</h3>
                    <p className="text-[12px]" style={{ color: 'rgba(240,237,232,0.4)' }}>{g.tagline}</p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-3">
                  <span className="flex items-center gap-1.5 text-[10px] tracking-[0.1em]" style={{ color: 'rgba(240,237,232,0.3)' }}>
                    <FileText size={10} strokeWidth={1.8} />
                    {g.pages} pages
                  </span>
                  <button className="flex items-center gap-1.5 text-[10px] font-medium tracking-[0.12em] transition-all duration-200 group-hover:gap-2" style={{ color: '#F0EDE8' }}>
        ...
        <div
          ...
          style={{ background: 'rgba(240,237,232,0.03)', border: '1px solid rgba(240,237,232,0.07)' }}
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 items-center">
            <div>
              <p className="text-[10px] tracking-[0.2em] font-medium mb-5" style={{ color: 'rgba(240,237,232,0.3)' }}>
              ...
              <p className="text-[14px] leading-[1.75]" style={{ color: 'rgba(240,237,232,0.42)' }}>
            ...
            <div>
              {sent ? (
                <div className="py-6">
                  <p className="text-[15px] font-light" style={{ color: '#F0EDE8' }}>You're in. Watch your inbox.</p>
                  <p className="text-[13px] mt-2" style={{ color: 'rgba(240,237,232,0.35)' }}>First guide lands within 24 hours.</p>
                </div>
              ) : (
                <form onSubmit={(e) => { e.preventDefault(); if (email) setSent(true) }} className="flex flex-col gap-3">
                  <input
                    ...
                    className="w-full rounded-full px-6 py-4 text-[14px] text-[#F0EDE8] outline-none transition-all duration-200"
                    style={{ background: 'rgba(240,237,232,0.05)', border: '1px solid rgba(240,237,232,0.1)' }}
                  ...
                  <button
                    type="submit"
                    className="w-full rounded-full py-4 text-[13px] font-medium tracking-[0.06em] bg-[#F0EDE8] text-[#060606] hover:bg-white transition-all duration-300 hover:scale-[1.01]"
              ...
              <p className="text-[11px] mt-4 text-center" style={{ color: 'rgba(240,237,232,0.22)' }}>
